...
window.Carrito =[];
if(localStorage.TotalDeProductosCarrito == "NaN"){
    window.totalcarrito = 0;
    window.totaldeproductos = 0;
    window.Carrito = [];
}else{
...
function SumRestToCart(operacion){
    if (operacion == "suma"){
        window.totaldeproductos ++;
    }else if(operacion == "resta"){
        window.totaldeproductos --;
    }
    document.getElementById("itemsnum").innerHTML = window.totaldeproductos;
    localStorage.setItem("TotalDeProductosCarrito", window.totaldeproductos);
    console.log("total de productos en carrito: " + window.totaldeproductos);
}

// Función para hacer el listado de productos del carrito
function ListadoCarrito(productoid){
    let productoEnCarrito;
    let cart = window.Carrito
    if (cart){
        productoEnCarrito = cart.find(element => element.id == productoid);
    }
    ...
    if(productoEnCarrito === undefined){
        ...
        window.Carrito.push(productoEnCarrito);
    }else{
        productoEnCarrito.cantidad ++;
    }
    window.totalcarrito = window.totalcarrito + productoEnCarrito.precio;
    localStorage.setItem("SumaPreciosCarrito", window.totalcarrito);
    localStorage.setItem("ListaCarrito", JSON.stringify(window.Carrito));
    SumRestToCart('suma');
}
